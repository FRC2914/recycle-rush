...
public class Keybind {
	...
	private keybindTriggerType triggerType;

	public enum keybindTriggerType {
		WHEN_PRESSED(false, true), WHEN_RELEASED(true, false), WHILE_PRESSED(
				true, true), WHILE_RELEASED(false, false);

		private boolean lastStateCriteria, currentStateCriteria;

		keybindTriggerType(boolean lastStateCriteria,
				boolean currentStateCriteria) {
		...
		public boolean test(boolean lastState, boolean currentState) {
			if (lastState == lastStateCriteria
					&& currentState == currentStateCriteria) {
				return true;
			}
			return false;
		}
	 ...
	 *            joystick button
	 * @param command
	 *            command to be run when button is pushed
	 */
	public Keybind(int button, String command) {
		this(button, command, keybindTriggerType.WHEN_PRESSED);
	}

	public Keybind(int button, String command, keybindTriggerType triggerType) {
		this.button = button;
	...
	/**
	 * called every tick to check if button has been pushed and if so call
	 * command
	 * 
	 * @param joystick
	 *            joystick to check button with
	 */
